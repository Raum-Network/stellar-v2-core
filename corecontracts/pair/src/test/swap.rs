...
#[test]
// #[should_panic(expected = "RaumFiPair: insufficient output amount")]
fn try_swap_amounts_zero() {
    ...
    assert_eq!(result, Err(Ok(PairError::SwapInsufficientOutputAmount)));
}

#[test]
// #[should_panic(expected = "RaumFiPair: negatives dont supported")]
fn try_swap_amount_0_negative() {
    let test = RaumFiPairTest::setup();
    test.env.budget().reset_unlimited();
    test.contract.initialize(&test.factory.address, &test.token_0.address, &test.token_1.address);
    let result = test.contract.try_swap(&-1, &1, &test.user);
    assert_eq!(result, Err(Ok(PairError::SwapNegativesOutNotSupported)));
}
...
#[test]
// #[should_panic(expected = "RaumFiPair: insufficient liquidity")]
fn try_swap_no_liquidity() {
    let test = RaumFiPairTest::setup();
    test.env.budget().reset_unlimited();
    test.contract.initialize(&test.factory.address, &test.token_0.address, &test.token_1.address);
    let result = test.contract.try_swap(&1, &1, &test.user);
    assert_eq!(result, Err(Ok(PairError::SwapInsufficientLiquidity)));
}

#[test]
// #[should_panic(expected = "RaumFiPair: invalid to")]
fn try_swap_to_token_0() {
    ...
    assert_eq!(result, Err(Ok(PairError::SwapInvalidTo)));
}
...
#[test]
// #[should_panic(expected = "RaumFiPair: insufficient input amount")]
fn try_swap_token_0_insufficient_input() {
    ...
    assert_eq!(result, Err(Ok(PairError::SwapInsufficientInputAmount)));
}
...
#[test]
// #[should_panic(expected = "RaumFiPair: K constant is not met")]
fn try_swap_token_0_low_sent() {
    ...
    let result = test.contract.try_swap(&0, &1000, &test.user);
    assert_eq!(result, Err(Ok(PairError::SwapConstantNotMet)));
}
