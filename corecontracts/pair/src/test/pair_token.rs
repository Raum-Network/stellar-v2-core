#![cfg(test)]
extern crate std;
use crate::test::{RaumFiPairTest, deposit::add_liquidity  };
use crate::pair_token::{PairToken, PairTokenClient};
use soroban_sdk::{
    symbol_short,
...
fn test() {
    ...
    assert_eq!(test.contract.balance(&user1), 1900);
    ...
    assert_eq!(test.contract.balance(&user1), 1300);
    ...
    assert_eq!(test.contract.balance(&user1), 1700);
    assert_eq!(test.contract.balance(&user2), 200);

    test.contract.transfer(&user1, &user3, &300);
    assert_eq!(test.contract.balance(&user1), 1400);
...
fn test_burn() {
    ...
    assert_eq!(test.contract.balance(&user1), 900);
    assert_eq!(test.contract.balance(&user2), 0);
}
...
fn test_zero_allowance() {
    ...
    let token_client = PairTokenClient::new(&test.env, &test.env.register_contract(&test.contract.address, PairToken {}));
