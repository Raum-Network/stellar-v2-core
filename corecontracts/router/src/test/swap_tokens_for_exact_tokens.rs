...
use crate::test::{RaumFiRouterTest, create_token_contract};
...
fn swap_tokens_for_exact_tokens_not_initialized() {
    ...
    assert_eq!(
        result,
        Err(Ok(RouterErrorsForLibrary::RouterNotInitialized))
    );
}
...
fn swap_tokens_for_exact_tokens_amount_out_negative() {
    ...
    assert_eq!(
        result,
        Err(Ok(RouterErrorsForLibrary::RouterNegativeNotAllowed))
    );
}
...
fn swap_tokens_for_exact_tokens_amount_in_max_not_enough() {
    ...
    assert_eq!(
        result,
        Err(Ok(RouterErrorsForLibrary::RouterExcessiveInputAmount))
    );
}
