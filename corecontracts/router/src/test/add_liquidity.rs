...
fn test_add_liquidity_deadline_expired() {
    ...
    assert_eq!(result, Err(Ok(RouterErrorsForLibrary::RouterDeadlineExpired)));
}
...
fn test_add_liquidity_deducted_amount_reserve() {
    let test = RaumFiRouterTest::setup_deducted_reserve();
    test.env.budget().reset_unlimited();
    test.contract.initialize(&test.factory.address);
...
fn insufficient_b_amount() {
    ...
    assert_eq!(result, Err(Ok(RouterErrorsForLibrary::RouterInsufficientBAmount)));
}
...
fn insufficient_a_amount() {
    ...
    assert_eq!(result, Err(Ok(RouterErrorsForLibrary::RouterInsufficientAAmount)));
}
